...
- **May 2, 2026** — **Ticket 10 (prod swap completion):** ran the manual prod-facing portion of ticket 10 outside the Claude Code session. Cleared `plaid_items`, `bank_accounts`, `transactions` on prod via `pnpm clear-plaid-data` (1 stale sandbox row truncated, CASCADE dropped any dependent matches; QBO, ledger, and `llm_logs` preserved as designed). Swapped `PLAID_ENV`, `PLAID_CLIENT_ID`, `PLAID_SECRET` to Trial Plan production values via `vercel env rm`/`add`. Redeployed via `vercel --prod`. Connected first real bank (BECU, Item 1 of 10) — 19 real transactions populated cleanly. Verified `v1:` prefix on the new prod-issued `access_token` via psql session-pooler check; encryption pipeline holds against real Plaid Production tokens. Ran matching against 19 BECU transactions × 7 QBO sandbox entries: zero matches, which is correct behavior given the two pools represent unrelated accounts (BECU checking vs QBO Mastercard ledger). v0 is now end-to-end live on real bank data at `reconcile-orcin.vercel.app`.
